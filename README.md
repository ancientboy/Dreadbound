...
项目处于**资源管理原型阶段**。当前包含中文版任务界面、覆盖整个副本的探索迷雾、可展开小地图、近战与病患敌人、绷带和回响碎片搜集、简化物品状态、手机触控，以及记录、供电和撤离任务路线；仍不包含正式美术和完整装备系统。

## 运行目标

安装 Godot 4 后，在 Project Manager 中导入 `project.godot` 并按 **F5**，或运行 `godot --path .`（部分系统命令名为 `godot4`）。电脑使用 WASD/方向键移动、空格或 J 攻击、E 交互、Q 使用绷带；手机和平板使用左侧摇杆及右侧攻击、交互、绷带按钮。建议横屏游玩。
...
6. **内容扩展（下一步）**：加入 The Crawler、基础远程武器和弹药资源。
7. **表现与平衡**：统一像素素材、灯光和音效、终端风格 UI、永久强化及 Web 性能优化。
