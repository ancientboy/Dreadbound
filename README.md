...
O1 美术试片已进入第二批 Review：首页使用正式 Key Art，行者与疗养院病患使用统一 48×64 四方向六帧素材，并改由 `Sprite2D` 稳定管理帧格；纹理异常时保留可见兜底轮廓。终末回廊现已接入 32×32 地面/墙体语言与正式门、终端、柜体、司仪基座 Prop；撬棍、手枪、霰弹枪及核心战斗反馈开始使用统一像素图集。试片必须通过桌面、手机横屏、完整测试与 Web 导出后才会从 `review` 升为 `approved`。
