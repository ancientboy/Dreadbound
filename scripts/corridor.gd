Warning: truncated output (original token count: 33425)
Total output lines: 2671
...
func _create_hub_navigation() -> void:
	...
	var entries := [
		...
		["dungeons", "副本", 5],
		["terminal", "终端", 6],
		# Keep avatar selection at the same level as the other core systems.
		# It used to exist only deep inside the terminal, making the new female
		# rig sample effectively undiscoverable after the hub navigation redesign.
		["avatar", "角色", 3],
	]
...
func _build_avatar_section() -> void:
	...
	for avatar_id in ["drifter_male", "drifter_female"]:
		...
		var active := GameState.player_avatar == avatar_id
