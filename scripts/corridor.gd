Warning: truncated output (original token count: 32608)
...
func _format_humanity_archive(assessment: Dictionary) -> String:
	...
	for dimension_id in HumanityProfile.DIMENSIONS:
		...
		if not knowledge.is_empty():
			knowledge_text = "%s（%s）" % [str(knowledge[0].get("t…2608 tokens truncated… %d · 余烬 %d" % [GameState.echo_shards, GameState.causality_fragments, GameState.synthesis_embers]
