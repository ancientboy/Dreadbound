...
func _run_test() -> void:
	...
	assert(state.action_ledger.events_for_run(action_code).size() == 4)
	assert(state.last_run.action_events.size() == 4)
	...
	assert(restored.action_ledger.events_for_run(action_code).size() == 4)
	assert(restored.world_state.processed_event_ids.size() == 4)
